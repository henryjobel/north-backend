...
export const createGreenCity = async (req, res) => {
  try {
    ...
    if (files?.locationTourVideo?.[0]) {
      const result = await uploadToCloudinary(files.locationTourVideo[0].buffer, "greenCity/locationTour", { resource_type: "auto" });
...
export const updateGreenCity = async (req, res) => {
  try {
    ...
    if (files?.locationTourVideo?.[0]) {
      if (greenCity.locationTourVideo && greenCity.locationTourVideo.includes("res.cloudinary.com")) {
        await deleteFromCloudinary(getPublicIdFromUrl(greenCity.locationTourVideo), "video");
      }
      const result = await uploadToCloudinary(files.locationTourVideo[0].buffer, "greenCity/locationTour", { resource_type: "auto" });
      updateData.locationTourVideo = result.url;
    } else if (body.locationTourVideo !== undefined) {
      if (
        greenCity.locationTourVideo &&
        greenCity.locationTourVideo !== body.locationTourVideo &&
        greenCity.locationTourVideo.includes("res.cloudinary.com")
      ) {
        await deleteFromCloudinary(getPublicIdFromUrl(greenCity.locationTourVideo), "video");
      }
    ...
    if (body.videoGallery !== undefined || files?.videoGalleryVideos?.length) {
      ...
      await deleteRemovedVideoGalleryUploads(greenCity.videoGallery, nextVideoGallery);
    ...
    if (files?.brochurePdf?.[0]) {
      ...
      const brochurePdfAsset = parseAssetField(body, "brochurePdfAsset");
      if (brochurePdfAsset) {
        if (greenCity.brochurePdf?.public_id && greenCity.brochurePdf.public_id !== brochurePdfAsset.public_id) {
          await deleteFromCloudinary(greenCity.brochurePdf.public_id, "raw");
        }
    ...
    if (files?.bookingPdf?.[0]) {
      ...
      const bookingPdfAsset = parseAssetField(body, "bookingPdfAsset");
      if (bookingPdfAsset) {
        if (greenCity.bookingPdf?.public_id && greenCity.bookingPdf.public_id !== bookingPdfAsset.public_id) {
          await deleteFromCloudinary(greenCity.bookingPdf.public_id, "raw");
        }
...
export const deleteGreenCity = async (req, res) => {
  try {
    ...
    await deleteVideoGalleryUploads(greenCity.videoGallery);
