...
export const updateSquareCity = async (req, res) => {
  try {
    ...
    if (files?.brochurePdf?.[0]) {
      ...
      const brochurePdfAsset = parseAssetField(body, "brochurePdfAsset");
      if (brochurePdfAsset) {
        if (squareCity.brochurePdf?.public_id && squareCity.brochurePdf.public_id !== brochurePdfAsset.public_id) {
          await deleteFromCloudinary(squareCity.brochurePdf.public_id, "raw");
        }
    ...
    if (files?.bookingPdf?.[0]) {
      ...
      const bookingPdfAsset = parseAssetField(body, "bookingPdfAsset");
      if (bookingPdfAsset) {
        if (squareCity.bookingPdf?.public_id && squareCity.bookingPdf.public_id !== bookingPdfAsset.public_id) {
          await deleteFromCloudinary(squareCity.bookingPdf.public_id, "raw");
        }
